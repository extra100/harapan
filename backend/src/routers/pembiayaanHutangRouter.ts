...
export const pembiayaanHutangRouter = express.Router()
import dayjs from 'dayjs'
import { PembiayaanHutangModel } from '../models/PembiayaanHutangModel';
pembiayaanHutangRouter.get(
  '/discount-summary',
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const discountSummary = await PembiayaanHutangModel.aggregate([
  ...
  asyncHandler(async (req: any, res: any) => {
    ...
    try {
      ...
      if (numericWarehouseId === 2) {
        transactions = await PembiayaanHutangModel.find(dateFilter)
      } else {
        transactions = await PembiayaanHutangModel.find({
...
pembiayaanHutangRouter.get(
  ...
  asyncHandler(async (req: any, res: any) => {
    ...
    const posData = await PembiayaanHutangModel.find({ contact_id: contact_id });
...
pembiayaanHutangRouter.get(
  ...
  asyncHandler(async (req: any, res: any) => {
    ...
    try {
      const posData = await PembiayaanHutangModel.find({
        'items.name': name,
      });
...
pembiayaanHutangRouter.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const transactions = await PembiayaanHutangModel.find(filter);
    res.json(transactions);
  })
);
...
pembiayaanHutangRouter.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (posData && posData.length > 0) {
      ...
      const posById = await PembiayaanHutangModel.findById(req.params.ref_number)
      if (posById) {
        res.json(posById)
      } else {
...
pembiayaanHutangRouter.post(
  '/',
  asyncHandler(async (req: any, res: any) => {
    const posData = req.body

    // Cek apakah sudah ada transaksi dengan memo yang sama
    const existingTransaction = await PembiayaanHutangModel.findOne({ memo: posData.memo })

    if (existingTransaction) {
      return res.status(400).json({ message: 'Transaksi dengan memo yang sama sudah ada!' })
    }

    const justPos = await PembiayaanHutangModel.create(posData)
...
pembiayaanHutangRouter.put(
  '/by-id/:ref_number',
  asyncHandler(async (req: any, res: any) => {
    const transaction = await PembiayaanHutangModel.findOne({
      ref_number: req.params.ref_number,
    })
...
pembiayaanHutangRouter.delete(
  '/:ref_number/witholdings/:witholdingId',
  asyncHandler(async (req: Request, res: Response) => {
    console.log('Request params:', req.params)

    const { ref_number, witholdingId } = req.params
    const transaction = await PembiayaanHutangModel.findOne({ ref_number })
...
export default pembiayaanHutangRouter
...
pembiayaanHutangRouter.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const transactions = await PembiayaanHutangModel.find(query, {
      witholdings: 1,
