...
export const pembiayaanRouter = express.Router()
import dayjs from 'dayjs'
import { PembiayaanModel } from '../models/PembiayaanModel';
pembiayaanRouter.get(
  '/discount-summary',
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const discountSummary = await PembiayaanModel.aggregate([
  ...
  asyncHandler(async (req: any, res: any) => {
    ...
    try {
      ...
      if (numericWarehouseId === 2) {
        transactions = await PembiayaanModel.find(dateFilter)
      } else {
        transactions = await PembiayaanModel.find({
...
pembiayaanRouter.get(
  ...
  asyncHandler(async (req: any, res: any) => {
    ...
    const posData = await PembiayaanModel.find({ contact_id: contact_id });
...
//     const posData = await PembiayaanModel.find({
...
pembiayaanRouter.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const transactions = await PembiayaanModel.find(filter);
    res.json(transactions);
  })
);
...
pembiayaanRouter.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (posData && posData.length > 0) {
      ...
      const posById = await PembiayaanModel.findById(req.params.ref_number)
      if (posById) {
        res.json(posById)
      } else {
...
pembiayaanRouter.post(
  '/',
  asyncHandler(async (req: any, res: any) => {
    const posData = req.body

    // Cek apakah sudah ada transaksi dengan memo yang sama
    const existingTransaction = await PembiayaanModel.findOne({ memo: posData.memo })

    if (existingTransaction) {
      return res.status(400).json({ message: 'Transaksi dengan memo yang sama sudah ada!' })
    }

    const justPos = await PembiayaanModel.create(posData)
...
pembiayaanRouter.put(
  '/by-id/:ref_number',
  asyncHandler(async (req: any, res: any) => {
    const transaction = await PembiayaanModel.findOne({
      ref_number: req.params.ref_number,
    })
...
export default pembiayaanRouter
...
//     const transactions = await PembiayaanModel.find(query, {
//       witholdings: 1,
//       trans_date: 1,
//       _id: 0,
//     }).lean();
