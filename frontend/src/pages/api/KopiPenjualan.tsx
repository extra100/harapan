...
import { useAddTransactionMutation, useGetTransactionByIdQuery } from '../../hooks/transactionHooks'
...
const KopiPenjualan = () => {
    ...
    const getPosDetail = allTransactions?.find(
      ...
      const dataItemsUntukTabel = getPosDetail?.items;
      const amount = getPosDetail?.amount;
      const JumlahTerbayar = getPosDetail?.down_payment;
    //   const idBank = getPosDetail?.bank_account_id;
      const jumlahHutang = getPosDetail?.due;
    //   console.log({dataItemsUntukTabel})
 
      const idOultet = getPosDetail?.warehouse_id;
const financeAccountIds = dataItemsUntukTabel?.map(item => item.finance_account_id) ?? [];
// console.log({financeAccountIds})
...
// console.log({dataStokBarang})
  ...
  // const { stokWarehouse } = useStokWarehouse('56,57,89', '2025-01-01');
...
// console.log({contacts})
 ...
 const { loading, takedueanContactStatusIdandMemoMny } =
  ...
  const [selectedFinanceAccountId, setSelectedFinanceAccountIds] = useState<
    any[]
  >([])

  useEffect(() => {
    if (user) {
      setSelectedWarehouseId(idOultet)
    }
  }, [user])
  const [dataSource, setDataSource] = useState<any[]>([])
  // useEffect(() => {
  //   if (financeAccountIds?.length > 0 && selectedWarehouseId !== null) {
  //     financeAccountIds?.forEach((productId: any) => {
  //       fetchStokBarang(productId, Number(selectedWarehouseId))
  //     })
  //   }
  // }, [financeAccountIds, selectedWarehouseId])

  const [selectedProductStocks, setSelectedProductStocks] = useState<any[]>([])

  // useEffect(() => {
  //   if (warehouseStock && financeAccountIds?.length > 0) {
  ...
  //         productId !== undefined &&
  //         qty !== undefined &&
  //         financeAccountIds?.includes(productId)
  //       ) {
  ...
  //   }
  // }, [warehouseStock, financeAccountIds, selectedWarehouseId])
  ...
  const priceAfterDiscount = dataItemsUntukTabel?.map((item) => item.price);
  const discountPercent = dataItemsUntukTabel?.map((item) => item.discount_percent);
  const [priceBeforeDiscount, setPriceBeforeDiscount] = useState<number[]>([]);

  useEffect(() => {
    if (priceAfterDiscount && discountPercent) {
      const calculatedPrices = priceAfterDiscount.map((price, index) => {
        const discount = discountPercent[index];
        if (discount !== undefined && discount > 0) {
          return price / (1 - discount / 100);
        }
        return price; // Jika tidak ada diskon, harga tetap sama
      });

      setPriceBeforeDiscount(calculatedPrices);
    }
  }, [priceAfterDiscount, discountPercent]); // Efek akan dipanggil ulang jika harga atau diskon berubah
...
//keloladiskon
  const handleDiscountChange = (value: string, record: any) => {
    ...
    const basePrice = priceBeforeDiscount.find(
      (price: any, index: any) => dataItemsUntukTabel![index]?.finance_account_id === record.finance_account_id
    );
    if (selectedDiscount) {
      const newPrice = calculateDiscount(basePrice as any, selectedDiscount.percentage)
        ...
        prev.map((item: any) =>
          item.finance_account_id === record.finance_account_id
            ? {
                ...
                subtotal: calculateSubtotal(basePrice as any, item.qty),
  ...
  const handleOkClick = () => {
    ...
    const selectedItems = barangs!.filter((item: any) =>
      selectedFinanceAccountId.includes(item.id)
    )
    ...
    setSelectedFinanceAccountIds(selectedFinanceAccountId.map(Number))
  }
...
//   const [selectedContact, setSelectedContact] = useState<any | null>(contactId as any);
useEffect(() => {
    if (getPosDetail?.contact_id) {
      setSelectedContact(getPosDetail.contact_id);
    }
  }, [getPosDetail]);

  useEffect(() => {
    if (getPosDetail?.items) {
...
//  console.log({nilaiPlatform})
  ...
  useEffect(() => {
    const calculateTotalSubtotal = () => {
      const totalAfterNewArray = dataSource.reduce((total, item) => total + item.subtotal, 0)
      setTotalSubtotal(totalAfterNewArray)
      setFormattedTotalSubtotal(totalAfterNewArray.toLocaleString('id-ID'))
    }
...
// console.log({refNumber})
  ...
  useEffect(() => {
    if (safeTotalReceivable > safeNilaiPlatform) {
      setAmountPaid(totalSubtotal);
    }
  }, [safeTotalReceivable, safeNilaiPlatform, totalSubtotal]);
  ...
  const handleSave = async () => {
    ...
    const invoiceData = {
      ...
      amount: 19779123,
      amount_after_tax: 0,
      warehouse_id: Number(selectedWarehouseId), // Convert to number here
      ...
      warehouses: [
        {
          warehouse_id: 5, // Convert to number here as well
          name: simpanGudang,
        },
      ],
    ...
    try {
      ...
      if (!result) {
        ...
        message.error('Gagal menyimpan invoice. Coba lagi.');
        setIsLoading(false);
  ...
  useEffect(() => {
    if (safeTotalReceivable > safeNilaiPlatform) {
      setAmountPaid((prevAmountPaid) => prevAmountPaid || safeNilaiPlatform);
    }
  }, [safeTotalReceivable, safeNilaiPlatform]);

  const formatNumber = (num: number): string => numeral(num).format('0,0');
  ...
  const columns = [
    ...
    
    {
        ...
        render: (text: any) => <div>{text ? formatNumber(text) : '-'}</div>,
      },
      {
        title: 'Total Diskon',
      ...
      },
    {
      ...
      },
      {
        ...
        align: 'right',
        render: (text: any, record: any) => {
          // Menghitung subtotal berdasarkan data di record
          const subtotal = record.qty * record.price;
          return (
            <div>
              {typeof subtotal === 'number' ? formatNumber(subtotal) : '0'}
            </div>
          );
        },
      }
,            
      ...
      <div className={`page-container ${isLoading ? 'loading' : ''}`}>
        ...
        >
          <div style={{ paddingBottom: '0px', border: 'red' }}>
            ...
            <Row gutter={16} style={{ marginBottom: '10px' }}>
              <Col span={12}>
                ...
                <span style={labelColonStyle}>:</span>
                <Input style={{ width: '70%' }} value={ref_number || refNumber} readOnly />
              </Col>
              <Col span={12}>
          ...
          {warehouseStock.length > 0 ? (
            ...
            <Table
              dataSource={dataItemsUntukTabel}
  ...
  <Col span={12}>
    ...
    //   customInput={Input}
    //   max={totalSubtotal}
      disabled={safeTotalReceivable > safeNilaiPlatform} // Disable jika kondisi terpenuhi
      style={{
        width: '70%',
        textAlign: 'right',
        fontSize: '24px',
    ...
    />
              ...
              <Row gutter={16} style={{ marginBottom: '10px' }}>
                <Col span={12}>
                  ...
                  <Input
                    value={formatRupiah(jumlahHutang)}
                    style={{
                      width: '70%',
                      textAlign: 'right',
                      fontSize: '16px',
